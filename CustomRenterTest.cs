...
{
public class CustomRenterTest : Indicator
{
	...
	protected override void OnRender(ChartControl chartControl, ChartScale chartScale)
	{
		...
		startPoint = new SharpDX.Vector2(start, halfHeight);
		endPoint = new SharpDX.Vector2(ChartPanel.X + ChartPanel.W, halfHeight);
...
namespace NinjaTrader.NinjaScript.Indicators
{
	...
	{
		private CustomRenterTest[] cacheCustomRenterTest;
		public CustomRenterTest CustomRenterTest()
		{
			return CustomRenterTest(Input);
		}

		public CustomRenterTest CustomRenterTest(ISeries<double> input)
		{
			if (cacheCustomRenterTest != null)
				for (int idx = 0; idx < cacheCustomRenterTest.Length; idx++)
					if (cacheCustomRenterTest[idx] != null &&  cacheCustomRenterTest[idx].EqualsInput(input))
						return cacheCustomRenterTest[idx];
			return CacheIndicator<CustomRenterTest>(new CustomRenterTest(), input, ref cacheCustomRenterTest);
		}
	}
}
...
namespace NinjaTrader.NinjaScript.MarketAnalyzerColumns
{
	...
	{
		public Indicators.CustomRenterTest CustomRenterTest()
		{
			return indicator.CustomRenterTest(Input);
		}

		public Indicators.CustomRenterTest CustomRenterTest(ISeries<double> input )
		{
			return indicator.CustomRenterTest(input);
		}
	}
